# 文档导航

优先阅读当前材料：

- [v073 修复验收结果、设计局限与已校验本地备份](../reports/prescale_v073_20260924/FINAL_REPORT.md)
- [v073 大规模实验前的有限修复与搜索对照（英文）](PRESCALE_V073_20260924.md)
- [v072 最终结果、方法结论与已校验本地备份](../reports/v072_final_20260924/FINAL_REPORT.md)
- [v072 评分基础设施修复记录（英文）](V072_REFERENCE_MEMORY_REPAIR_20260924.md)
- [v072 今日结果、暂停位置与关机备份](../reports/v072_analysis_20260923/ANALYSIS_AND_SHUTDOWN_20260923.md)
- [v072 后续搜索、选择、终评与恢复协议（英文）](EXPERIMENT_V072_CONTINUATION.md)
- [v072 实际启动范围、保存与恢复机制（英文）](EXPERIMENT_V072.md)

- [v072 验证问题、对照与启动前检查（英文）](VALIDATION_V072.md)
- [实验前模型决策](MODEL_DECISION_20260923.md)
- [跨任务 v072 计划与 token 预算](MULTIDOMAIN_V072_EXPERIMENT_PLAN_20260923.md)
- [三档模型选择与相关研究](THREE_TIER_RESEARCH_SELECTION_V071.md)
- [问题—修改—验证对应清单](ISSUE_VALIDATION_MATRIX_20260923.md)
- [下一轮修改方案](NEXT_REVISION_PLAN_20260923.md)
- [v071 数学筛选复核](../reports/qwen38_v071_20260923/ANALYSIS_AND_RECOMMENDATIONS.md)

`EXPERIMENT_V051.md`、`EVALUATION_V052.md`、`FIXED_VALIDATION_V060.md`、`RECOVERY_V052.md`、`REVISION_V050.md` 等保留历史实验上下文；其中模型、绝对路径、预算和运行指令不代表下一轮默认配置。`ENGINEERING_RECORD.md` 记录原始方案之外的实现补充。

服务器交接提示、个人对话备忘、完整原始日志和旧文档草稿只保留在本地，不随 Git 仓库发布。因此历史文档中指向这些归档的链接可能需要原实验目录；仓库包含代码和精选证据摘要，不是完整实验数据归档。旧文件中的状态以其记录日期为准，当前状态以根目录 README 和对应最新版本文档为准。
